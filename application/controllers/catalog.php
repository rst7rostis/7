...
class Catalog extends CI_Controller {
		...
		{		
			...
			$this->load->view('face');
			$this->load->view('current_vid', $this->data_inf);
		}		
		
		public function findvideo()
		{
		$this->load->model('face_model');		
		$vid = scandir('l:\\vid\\mbt\\mts');
			$data['comment']=0;
			$data['type']=0;
			$data['vidsize']=0;
			$data['vidtime']=0;
		foreach($vid as $k=>$v){
			if($v=='.' or $v=='..' or $v=='v' or $v=='vid' or $v=='e' or $v=='pron' or $v=='clips' or $v=='mts'){
				continue;
			}else{
				$data['title']=$v;
				$data['link'] = 'l:\\vid\\mbt\\mts\\'.$v;
				$this->face_model->dumbindb($data);
			}
					}
		...
		public function search_in_db(){
			...
			$data['type'] = $this->input->post('type');
			$this->load->model('face_model');
			$this->e=$this->face_model->search_i_db($data['title'], $data['type']);
			$this->load->view('search', $this->e);
		}
